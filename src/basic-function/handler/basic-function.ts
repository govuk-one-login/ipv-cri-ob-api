...
import {
  ClientIDToTokenProfileMapping,
  OAuthClientID
} from '@src/token-rotator/model/token-profile'
...
export class BasicFunction {
  ...
  private async process(event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
    const clientID: OAuthClientID = OAuthClientID.IPV_CORE_STUB
    const profile = ClientIDToTokenProfileMapping[clientID]
