...
import {
  createIssueCredentialService,
  type IssueCredentialService
} from '@src/issue-credential/service/issue-credential-service'
...
describe('issue-credential-service', () => {
  ...
  beforeEach(() => {
    ...
    service = createIssueCredentialService(collaborators)
  ...
  it('throws SessionNotFoundError when the access token has no session', async () => {
    ...
    await expect(service({ accessToken: 'abc.def.ghi' })).rejects.toBeInstanceOf(
      SessionNotFoundError
    )

    expect(collaborators.sessionRepository.findByAccessToken).toHaveBeenCalledWith('abc.def.ghi')
    expect(collaborators.personIdentityRepository.findBySessionId).not.toHaveBeenCalled()
  ...
  it('throws PersonDetailsNotFoundError when person details are missing', async () => {
    collaborators.personIdentityRepository.findBySessionId.mockResolvedValue(undefined)
  ...
  it('builds, signs and returns the credential on the happy path', async () => {
    await expect(service({ accessToken: 'abc.def.ghi' })).resolves.toEqual({
      credential: 'signed.jwt.value'
    })

    expect(collaborators.personIdentityRepository.findBySessionId).toHaveBeenCalledWith(
      'session-123'
    )
  ...
  it('appends session identifiers to the logger context', async () => {
    await service({ accessToken: 'abc.def.ghi' })
